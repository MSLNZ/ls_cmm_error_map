...
this version splits the plotting and data updating
"""
...
def plot_update(params, mag, lineplots, plotw):
    RP = np.array(
        [
            [1.0, 0.0, 0.0, 100.0],
            [0.0, 0.0, 1.0, 50.0],
    ...
    data = single_grid_plot_data(dxy, mag)
    di = 0
    for datum, plot in zip(data, lineplots):
        x, y = plot.getData()
        print(di)
        di += 1
...
lineplots, plotw = plot_ballplate()
plot_update(params, mag, lineplots, plotw)
layout.addWidget(plotw)
mw.show()
